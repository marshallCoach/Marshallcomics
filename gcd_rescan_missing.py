#!/usr/bin/env python3
"""
gcd_rescan_missing.py — incremental rescan of the GCD dump for inventory
...
Usage:  python3 gcd_rescan_missing.py /path/to/2026-07-15.sql
"""
...
def main():
    if len(sys.argv) < 2:
        print("usage: python3 gcd_rescan_missing.py /path/to/dump.sql"); sys.exit(1)
    dump = sys.argv[1]
    ...
    missing = sorted(t for t in titles if t not in have)
    print(f"Inventory titles without a matched GCD series: {len(missing)}", file=sys.stderr)
