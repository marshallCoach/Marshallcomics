  ...
  return (
    <div style={{ maxWidth:1100, margin:"0 auto", padding:"0 0 60px" }}>
      ...
      <div style={{ background:"#1a0a0a", padding:"20px 20px 16px", borderBottom:"3px solid var(--red)" }}>
        <div style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"1.75rem", letterSpacing:"4px", color:"#fff", lineHeight:1 }}>
          SELLER COMMAND CENTER
        </div>
        <div style={{ fontSize:"0.875rem", letterSpacing:"2px", color:"rgba(255,255,255,0.4)", marginTop:4, fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
        ...
        <div style={{ display:"flex", gap:0, marginTop:14, flexWrap:"wrap" }}>
          {[
            ...
            <div key={s.lbl} style={{ flex:"1 1 120px", padding:"10px 14px", borderRight:"1px solid rgba(255,255,255,0.08)" }}>
              <div style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"1.75rem", color:s.color, letterSpacing:"1px" }}>{s.val}</div>
              <div style={{ fontSize:"0.875rem", color:"rgba(255,255,255,0.35)", letterSpacing:"1.5px", textTransform:"uppercase", marginTop:2 }}>{s.lbl}</div>
              <div style={{ fontSize:"0.875rem", color:"rgba(255,255,255,0.2)", marginTop:1 }}>{s.sub}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display:"flex", borderBottom:"2px solid var(--border)", background:"var(--surface)", overflowX:"auto" }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", letterSpacing:"2px", fontSize:"0.875rem",
      ...
      {tab === "action" && (
        <div>
          ...
          <div style={{ background:"var(--surface2)", padding:"12px 16px", borderBottom:"1px solid var(--border)",
            ...
            <div>
              <div style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", color:"var(--green-text)", letterSpacing:"1px" }}>
                {fmtMoney(actionTotals.doneVal)} secured
              </div>
              <div style={{ fontSize:"0.875rem", color:"var(--muted)", letterSpacing:"1px" }}>
                ...
                <div key={item.id}
                  ...
                  <div style={{ padding:"10px 14px 8px", cursor:"pointer", display:"flex", gap:10, alignItems:"flex-start" }}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:4, flexWrap:"wrap" }}>
                        ...
                        <span style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"2px",
                          background:pColor+"18", color:pColor, padding:"1px 7px", borderRadius:2 }}>
                          {item.deadline}
                        </span>
                        <span style={{ fontSize:"0.875rem", color:"var(--muted)", letterSpacing:"1px" }}>
                          {CAT_ICON[item.category]} {item.category.toUpperCase()}
                        </span>
                        {isDone && <span style={{ fontSize:"0.875rem", color:"var(--green-text)", letterSpacing:"1px",
                          fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>✓ DONE</span>}
                      </div>
                      ...
                      {item.book && (
                        <div style={{ fontSize:"0.875rem", color:"var(--muted2)", marginTop:3 }}>
                    ...
                    <div style={{ flexShrink:0, textAlign:"right" }}>
                      ...
                      {item.costMin > 0 && (
                        <div style={{ fontSize:"0.875rem", color:"var(--muted)", marginTop:1 }}>
                          spend {fmtMoney(item.costMin)}{item.costMin !== item.costMax ? `–${fmtMoney(item.costMax)}` : ""}
                        </div>
                      )}
                      <div style={{ marginTop:5 }}>
                        <RoiBar bars={bars} color={roiColor} />
                        <div style={{ fontSize:"0.875rem", color:roiColor, marginTop:2, textAlign:"right" }}>
                    ...
                    <div style={{ display:"flex", gap:3, alignItems:"center" }}>
                      {["Effort", `${item.effortHours}h`].map((v,i) => (
                        <span key={i} style={{ fontSize:"0.875rem",
                          ...
                          fontFamily: i === 1 ? "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" : undefined,
                  ...
                  {isOpen && (
                    <div style={{ padding:"10px 14px 12px", borderTop:"1px solid var(--border)",
                      background:"var(--surface2)", fontSize:"0.875rem", color:"var(--muted2)", lineHeight:1.6,
        ...
        <div>
          ...
          <div style={{ background:"var(--surface2)", padding:"12px 16px", borderBottom:"1px solid var(--border)" }}>
            <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
              <input value={ebayQuery} onChange={e=>{setEbayQuery(e.target.value);clearEbay();}}
                ...
                style={{ flex:"1 1 200px", padding:"7px 12px", border:"1.5px solid var(--border)",
                  borderRadius:4, fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem",
                  background:"var(--surface)", color:"var(--text)" }} />
              {ebayQuery && <button onClick={()=>setEbayQuery("")}
                style={{ padding:"7px 12px", border:"1.5px solid var(--border)", borderRadius:4,
                  background:"transparent", color:"var(--muted)", cursor:"pointer", fontSize:"0.875rem" }}>
                Clear
              </button>}
              <div style={{ display:"flex", gap:4 }}>
            ...
            <div style={{ display:"flex", gap:20, flexWrap:"wrap", alignItems:"center", marginTop:10 }}>
              <label style={{ display:"flex", gap:6, alignItems:"center", fontSize:"0.875rem", color:"var(--muted2)" }}>
                eBay fee
          ...
          {ebayTotals.count > 0 && (
            <div style={{ position:"sticky", top:94, zIndex:20, background:"#1a0a0a",
              ...
              display:"flex", gap:20, flexWrap:"wrap", alignItems:"center" }}>
              <span style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", color:"#fff", letterSpacing:"1.5px" }}>
              ...
              {[
                ...
                <div key={s.lbl}>
                  <span style={{ fontSize:"0.875rem", color:"rgba(255,255,255,0.4)", letterSpacing:"1.5px" }}>{s.lbl} </span>
                  <span style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", color:s.color, letterSpacing:"1px" }}>{s.val}</span>
                </div>
              ))}
              <button onClick={clearEbay}
                style={{ marginLeft:"auto", fontSize:"0.875rem", padding:"4px 10px", border:"1.5px solid rgba(255,255,255,0.2)",
          ...
          <div style={{ overflowX:"auto" }}>
              ...
              <thead>
                <tr style={{ background:"var(--surface2)", borderBottom:"2px solid var(--border)" }}>
                  ...
                  {["Title","Box","Issue","VF Value","eBay Fee","Net Profit","Key"].map(h => (
                    <th key={h} style={{ padding:"8px 10px", textAlign:"left", fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",
                      letterSpacing:"1.5px", fontSize:"0.875rem", color:"var(--muted)", fontWeight:400 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={idx} onClick={() => toggleEbay(idx)}
                      ...
                      <td style={{ padding:"7px 10px", color:"var(--muted2)" }}>
                        <span style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", letterSpacing:"1px", fontSize:"0.875rem",
                      ...
                      <td style={{ padding:"7px 10px", color:"#f87171", fontSize:"0.875rem" }}>
                        {vfN > 0 ? `-$${fee.toFixed(2)}` : "—"}
                      </td>
                      ...
                      <td style={{ padding:"7px 10px" }}>
                        {isKey && <span style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"1.5px",
            ...
            {ebayResults.length > 200 && (
              <div style={{ padding:"10px 16px", fontSize:"0.875rem", color:"var(--muted)", textAlign:"center" }}>
      ...
      {tab === "cgc" && (
        <div>
          ...
          <div style={{ background:"var(--surface2)", padding:"12px 16px", borderBottom:"1px solid var(--border)" }}>
            <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
              <span style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"2px", color:"var(--muted)" }}>CGC TIER</span>
                ...
                {CGC_QUEUE.map(book => {
                  ...
                  return (
                    <tr key={book.id} onClick={() => toggleCgc(book.id)}
                      ...
                      <td style={{ padding:"8px 10px", color:"var(--muted)", fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",
                        letterSpacing:"1px", fontSize:"0.875rem" }}>{book.id}</td>
                      <td style={{ padding:"8px 10px" }}>
                        <div style={{ fontWeight:600, color:"var(--text)" }}>{book.title}</div>
                        <div style={{ fontSize:"0.875rem", color:"var(--muted2)", marginTop:1 }}>{book.subtitle}</div>
                      </td>
                      <td style={{ padding:"8px 10px" }}>
                        <span style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", letterSpacing:"1px", fontSize:"0.875rem",
                          background:"#7a5c3a18", border:"1.5px solid #7a5c3a", color:"#7a5c3a",
      ...
      {tab === "sell30" && (
        <div>
          ...
          <div style={{ background:"var(--surface2)", padding:"12px 16px", borderBottom:"1px solid var(--border)",
            ...
            <span style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"2px", color:"var(--muted)" }}>SORT</span>
            ...
            <span style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"2px", color:"var(--muted)" }}>FILTER</span>
            ...
            {sortedSell.map(book => {
              ...
              return (
                <div key={book.rank} style={{ flex:"1 1 280px", background:"var(--surface)",
                  ...
                  <div style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"1px",
                    color:"var(--text)", lineHeight:1.2 }}>
                    {book.title} {book.issue}
                  </div>
                  <div style={{ fontSize:"0.875rem", color:"var(--muted2)", marginTop:3, lineHeight:1.4 }}>
                    {book.subtitle}
                  </div>
                  <div style={{ fontSize:"0.875rem", color:"var(--muted)", marginTop:4 }}>Box {book.box}</div>
                  <div style={{ display:"flex", gap:10, marginTop:8 }}>
                    <div>
                      <div style={{ fontSize:"0.875rem", color:"var(--muted)", letterSpacing:"1.5px" }}>VF VALUE</div>
                      <div style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", color:"var(--muted2)", letterSpacing:"1px" }}>{vfStr}</div>
                    </div>
                    <div style={{ borderLeft:"1px solid var(--border)", paddingLeft:10 }}>
                      <div style={{ fontSize:"0.875rem", color:"var(--muted)", letterSpacing:"1.5px" }}>EXPECTED</div>
                      <div style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", color:"var(--green-text)", letterSpacing:"1px" }}>
                        {fmtMoney(book.expectedMin)}–{fmtMoney(book.expectedMax)}
                      </div>
                    </div>
                    <div style={{ borderLeft:"1px solid var(--border)", paddingLeft:10, flex:1 }}>
                      <div style={{ fontSize:"0.875rem", color:"var(--muted)", letterSpacing:"1.5px" }}>PLATFORM</div>
                      <div style={{ fontSize:"0.875rem", color:"var(--text)", marginTop:1 }}>{book.platform}</div>
                    </div>
                  </div>
                  <div style={{ marginTop:8, padding:"5px 8px", background: pColor+"11",
                    border:`1px solid ${pColor}33`, borderRadius:3, fontSize:"0.875rem", color:pColor, lineHeight:1.4 }}>
                    → {book.action}
                  </div>
                </div>
              );
