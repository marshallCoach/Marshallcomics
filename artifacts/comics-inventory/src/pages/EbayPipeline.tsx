    ...
    <div style={{ padding:"24px 24px 80px", maxWidth:720, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:28, padding:"12px 14px", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:6 }}>
        {[
          ...
          <div key={c.text} style={{ display:"flex", alignItems:"center", gap:5, fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"1.5px", padding:"2px 8px", borderRadius:3, background:`${c.color}12`, color:c.color }}>
            ...
            <div key={step.num} style={{ display:"grid", gridTemplateColumns:"40px 1fr", gap:"0 14px", marginBottom:6, position:"relative" }}>
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center", paddingTop:4, flexShrink:0, position:"relative", zIndex:1 }}>
                <div style={{
                  ...
                  fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", color,
              ...
              <div>
                <div style={{
                  ...
                  <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10, marginBottom:6 }}>
                    <div style={{ fontSize:"0.875rem", fontWeight:600, color:"var(--text)", lineHeight:1.3 }}>{step.title}</div>
                    <span style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"1px", padding:"2px 7px", borderRadius:3, flexShrink:0, background:`${color}18`, color }}>
                      {statusLabel[step.status]}
                    </span>
                  </div>
                  <div style={{ fontSize:"0.875rem", color:"var(--muted)", lineHeight:1.55 }}>{step.body}</div>
                  {step.failures.length > 0 && (
                    <div style={{ display:"flex", flexWrap:"wrap", gap:4, marginTop:8 }}>
                      {step.failures.map(f => (
                        <span key={f} style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", padding:"2px 7px", borderRadius:3, background:"#e0555514", color:"#e05555", border:"1px solid #e0555525" }}>✗ {f}</span>
                      ))}
                    </div>
                  )}
                </div>
      ...
      {/* Audit table */}
      <div style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"2.5px", color:"var(--muted)", marginBottom:12 }}>FAILURE AUDIT — FIRST-TIME SUCCESS RATE BY STEP</div>
          ...
          <thead>
            <tr>
              {["Step", "Failures", "Root Cause", "Fixed By"].map(h => (
                <th key={h} style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"1px", color:"var(--muted)", textAlign:"left", padding:"6px 10px 8px", borderBottom:"1px solid var(--border)", fontWeight:600 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {auditRows.map(r => (
              <tr key={r.step}>
                <td style={{ padding:"8px 10px", borderBottom:"1px solid var(--border)", fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"1px", color:"var(--muted)", whiteSpace:"nowrap" }}>{r.step}</td>
                ...
                <td style={{ padding:"8px 10px", borderBottom:"1px solid var(--border)", color:"var(--muted)", lineHeight:1.4, fontSize:"0.875rem" }}>{r.cause}</td>
                <td style={{ padding:"8px 10px", borderBottom:"1px solid var(--border)", color:"var(--text)", lineHeight:1.4, fontSize:"0.875rem" }}>{r.fix}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div style={{ padding:"16px 18px", background:"var(--surface)", borderLeft:"3px solid #4dabf7", borderRadius:"0 6px 6px 0", border:"1px solid var(--border)" }}>
        <div style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"2px", color:"#4dabf7", marginBottom:14 }}>PERMANENT FIXES — DO THESE ONCE ON MAC</div>
        {fixes.map(f => (
          <div key={f.label} style={{ display:"grid", gridTemplateColumns:"180px 1fr", gap:"8px 14px", padding:"8px 0", borderBottom:"1px solid var(--border)", fontSize:"0.875rem", lineHeight:1.5 }}>
            <div style={{ fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", fontSize:"0.875rem", letterSpacing:"1px", color:"var(--muted)", paddingTop:1 }}>{f.label}</div>
