  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "18px 18px 60px" }}>
      ...
      <div style={{ marginBottom: 14, fontFamily: "'Bebas Neue',sans-serif",
        fontSize: "1.2rem", letterSpacing: "2px", color: "var(--muted)" }}>
              ...
              return (
                <div key={sec.key}>
                  ...
                  <div style={{
                    ...
                    <span style={{ fontSize: "1.2rem", color:"var(--muted2)", fontFamily:"'Crimson Pro',serif" }}>
                      {sec.subtitle}
                    </span>
                    <span style={{
                      marginLeft:"auto", fontFamily:"'Bebas Neue',sans-serif",
                      fontSize: "1.2rem", letterSpacing:"1.5px", color:"var(--muted)",
                    }}>
              ...
              return (
                <div key={status} style={{ marginBottom: 16 }}>
                  <div style={{
                    ...
                    <span style={{ fontSize: "0.82rem", color: "var(--muted2)", marginLeft: 4 }}>
                    ...
                    {group.map((c, i) => (
                      <div key={i} style={{
                        ...
                        {(c.Key || "").toUpperCase() === "YES" && (
                          <span style={{
                            fontSize: "0.68rem", background: meta.color, color: "#fff",
      ...
      {hoveredBox && hoveredBoxData && (
        <div style={{
          ...
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"1.2rem", color:"var(--red)", letterSpacing:"2px", lineHeight:1 }}>{hoveredBoxData.Num}</div>
          <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize: "1.2rem", color:"var(--muted2)", letterSpacing:"1px", marginBottom:10 }}>
          ...
          <div style={{ display:"flex", gap:20, marginBottom:10 }}>
            {([
              ...
              <div key={s.lbl} style={{ textAlign:"center" }}>
                <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:"1.4rem", color:"var(--red)", lineHeight:1 }}>{s.val}</div>
                <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize: "1.2rem", letterSpacing:"2px", color:"var(--muted)", marginTop:1 }}>{s.lbl}</div>
              </div>
            ))}
          </div>
          {hoveredBoxData.Notes && (
            <div style={{ fontSize: "1.2rem", color:"var(--muted2)", lineHeight:1.5, borderTop:"1px solid var(--border)", paddingTop:8 }}>
              {hoveredBoxData.Notes}
            </div>
          )}
          {hoveredBoxData.YearRange && (
            <div style={{ fontSize: "1.2rem", color:"var(--muted)", marginTop:6 }}>
              {hoveredBoxData.YearRange}
            </div>
          )}
          <div style={{ fontSize: "1.2rem", color:"var(--muted)", marginTop:8, fontFamily:"'Bebas Neue',sans-serif", letterSpacing:"1.5px", opacity:0.65 }}>
            CLICK TO EXPLORE →
          </div>
        </div>
      )}
      ...
      {!selectedBox && (
        <div style={{ textAlign: "center", color: "var(--muted)", padding: "60px 20px",
          fontFamily: "'Bebas Neue',sans-serif", fontSize: "1.2rem", letterSpacing: "2px" }}>
        ...
        <div ref={detailRef}>
          ...
          <div style={{ background: "var(--surface)", border: "1.5px solid var(--border)",
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: "1.2rem", fontWeight: 600, color: "var(--brown-light)", marginTop: 3 }}>
                {selectedBoxData.Label.replace(/^Box \d+ — /i, "")}
              </div>
              <div style={{ fontSize: "1.2rem", color: "var(--muted)", marginTop: 4 }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
            {(["visual", "runs", "comics"] as const).map(v => (
              <button key={v} onClick={() => { setView(v); setSelectedTitle(null); }} style={{
                ...
                fontFamily: "'Bebas Neue',sans-serif", fontSize: "1.2rem", letterSpacing: "1.5px",
          ...
          {view === "visual" && sorted && titleGroups.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10, padding: "8px 10px", background: "var(--surface2)", borderRadius: 6, border: "1px solid var(--border)" }}>
              <div style={{ width:"100%", fontFamily:"'Bebas Neue',sans-serif", fontSize: "1.2rem", letterSpacing:"2px", color:"var(--muted)", marginBottom:3 }}>
                ...
                <div style={{ background: "var(--surface)", border: "1.5px solid var(--border)",
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    ...
                    <button onClick={() => setSelectedTitle(null)} style={{
                      ...
                      color: "var(--muted)", fontSize: "1.2rem", lineHeight: 1, padding: 0 }}>✕</button>
                  </div>

                  <div style={{ fontSize: "1.2rem", fontWeight: 700, color: "var(--brown-light)",
                    ...
                    return (
                      <div style={{ marginBottom: 12 }}>
                        {[
                          ...
                          <div key={r.l} style={{ display: "flex", gap: 8, marginBottom: 3, fontSize: "1.2rem" }}>
                            <span style={{ color: "var(--muted)", minWidth: 60, flexShrink: 0, fontSize: "1.2rem" }}>{r.l}</span>
                  ...
                  {selectedTitleGroup.issues.some(c => c.Key_Reason) && (
                    <div style={{ marginTop: 12 }}>
                      ...
                      {selectedTitleGroup.issues.filter(c => c.Key_Reason).map((c, i) => (
                        <div key={i} style={{ fontSize: "1.2rem", color: "var(--text2)",
                    ...
                    <div key={i} style={{
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <div style={{ display: "flex", gap: 4 }}>
                          {isKey    && <span className="badge bkey"  style={{ fontSize: "1.2rem" }}>KEY</span>}
                          {isSigned && <span className="badge bgold" style={{ fontSize: "1.2rem" }}>SIGNED</span>}
                          {c.Platform && <span className="badge bb"  style={{ fontSize: "1.2rem" }}>{c.Platform}</span>}
                        </div>
                      </div>
                      ...
                      <div style={{
                        ...
                        fontSize: "1.2rem", color: "var(--brown-light)", lineHeight: 1.25,
                      }}>{c.Title}</div>

                      {/* Issue + Vol + Year */}
                      <div style={{ fontSize: "1.2rem", color: "var(--muted2)" }}>
                        {(()=>{const ip=parseIssueParts(c.Issue);return<>{ip.main}{ip.legacy&&<span style={{fontSize: "1.2rem",fontFamily:"'Bebas Neue',sans-serif",letterSpacing:"1px",background:"#e8f0fe",color:"#1d4ed8",borderRadius:3,padding:"1px 5px",marginLeft:4}}>{ip.legacy}</span>}</>;})()}
                      ...
                      {/* Publisher */}
                      {c.Publisher && (
                        <div style={{ fontSize: "1.2rem", color: "var(--muted)" }}>{c.Publisher}</div>
                      )}

                      {/* Writer / Artist */}
                      {(c.Writer || c.Artist) && (
                        <div style={{ fontSize: "1.2rem", color: "var(--muted2)", lineHeight: 1.4 }}>
                      ...
                      {/* Values */}
                      {(nmVal || vfVal) && (
                        <div style={{ display: "flex", gap: 10, fontSize: "1.2rem", marginTop: 2 }}>
      ...
      {hoveredSpine && (
        <div style={{
          ...
          <div style={{
            ...
            {hoveredSpine.year && <span style={{color:"var(--muted2)", fontSize: "1.2rem"}}>{hoveredSpine.year}</span>}
            {hoveredSpine.isKey && (
              <span style={{
                background:"#fff8e0", color:"#8a6000",
                border:"1px solid #d4a800", borderRadius:3,
                padding:"1px 6px", fontSize: "1.2rem",
          ...
          {hoveredSpine.publisher && (
            <div style={{ fontSize: "1.2rem", color:"var(--muted)", marginTop:4, fontFamily:"'Bebas Neue',sans-serif", letterSpacing:"1px" }}>
              {hoveredSpine.publisher}
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: 6,
      ...
      <div onClick={() => setOpen(o => !o)} style={{
        ...
        cursor: "pointer" }}>
        <div style={{ flex: 1 }}>
          <span style={{ fontSize: "1.2rem", fontWeight: 600, color: "var(--brown-light)" }}>{group.title}</span>
          <span style={{ fontSize: "1.2rem", color: "var(--muted)", marginLeft: 8 }}>
        ...
        <div style={{ display: "flex", gap: 5 }}>
          {group.keyCount > 0 && (
            <span style={{ background: "#fff8e0", color: "#8a6000", border: "1px solid #d4a800",
              borderRadius: 3, padding: "1px 6px", fontSize: "1.2rem",
              fontFamily: "'Bebas Neue',sans-serif", letterSpacing: "1px" }}>
              {group.keyCount} KEY
            </span>
          )}
        ...
        <span style={{ color: "var(--muted)", fontSize: "1.2rem", flexShrink: 0 }}>{open ? "▲" : "▼"}</span>
      </div>
            ...
            {group.issues.map((c, i) => {
              ...
              return (
                <div key={i} title={c.Key_Reason || c.Arc} style={{
                  ...
                  borderRadius: 4, padding: "3px 8px", fontSize: "1.2rem",
