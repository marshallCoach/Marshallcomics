#!/usr/bin/env python3
"""
brb_avengers_legacy.py — fill blank Avengers credits using the Fandom legacy
...
Usage:
    python3 brb_avengers_legacy.py            # dry-run, no writes
    python3 brb_avengers_legacy.py --apply
"""
...
MAP = os.path.join(ROOT, "avengers_legacy_map.json")
...
def main():
    ...
    for row in ws.iter_rows(min_row=2):
        ...
        if title.lower().replace("the ", "") != "avengers":
            continue
        if blank(row[wi].value) or blank(row[ai].value) or blank(row[ci].value):
            targets.append(row)
    if args.limit:
        targets = targets[: args.limit]
    print(f"  Avengers rows needing credits: {len(targets)}")
    ...
    for row in targets:
        ...
        cands = []
        if iss in legacy:
            cands.append(("legacy", legacy[iss]["page"]))
        if vol and vol.isdigit():
            cands.append(("declared", f"Avengers Vol {vol} {iss}"))
        pick = pickkind = None
        ...
        if not pick:
            rejected += 1
            print(f"    #{iss:<5} yr={yr} vol={vol} -> no candidate corroborates the year", flush=True)
            continue
        ...
        d = pick["year"] - yr
        print(f"    #{iss:<5} yr={yr} vol={vol} -> {pickkind:<8} "
    ...
    if args.apply:
        out = args.out or xlsx.replace(".xlsx", "_AVENGERS.xlsx")
        wb.save(out)
        print(f"  Written: {out}")
    else:
