...
QUEUE_PATH        = "final_run_queue.pkl"
...
def main():
    ...
    for _, run in queue.iterrows():
        ...
        try:
            if is_dc:
                # Try each issue independently via DC Fandom URL construction
                # Group by consecutive issue ranges to batch sensibly
                filled_count = 0
                writers_found = {}
                pencilers_found = {}
                ...
                # Apply fills — only where we got confident data
                ...
                # Non-DC: use Claude web search (same as v1)
